  ...
  <div class="order">
    ...
    <div style="margin-left: 50px">
      <el-tabs v-model="activeName"  @tab-click="getAllOrderByStatus()">
        ...
        <el-tab-pane  name="0"> <span slot="label">待付款<span style="margin-left: 5px;color: #ff6700;">{{orderNum('0')}}</span></span></el-tab-pane>
        <el-tab-pane  name="1"> <span slot="label">待发货<span style="margin-left: 5px;color: #ff6700;">{{orderNum('1')}}</span></span></el-tab-pane>
        <el-tab-pane  name="2"> <span slot="label">待收货<span style="margin-left: 5px;color: #ff6700;">{{orderNum('2')}}</span></span></el-tab-pane>
        <el-tab-pane  name="3"> <span slot="label">待评价<span style="margin-left: 5px;color: #ff6700;">{{orderNum('3')}}</span></span></el-tab-pane>
    ...
    <div class="order-content" v-if="(allOrder!=null)&&(this.activeName!='0')">
      <div class="content" v-for="item in allOrder" :key="item.id">
        <ul>
          <!-- 我的订单表头 -->
          <li class="order-info">
            <div class="order-time"> {{item.createTime| dateFormat1}}</div>
            <div class="order-id">订单号:{{item.id}}</div>
            <!--倒计时  支付超过半小时就自动取消该订单-->
            <div class="order-status" >状态:

                <span v-if="item.itemStatus==0" style="color: #f91424">待付款</span>
                ...
                <span v-if="item.itemStatus==5" style="color: #ff330a">交易完成</span>
          ...
          <li class="product-list">
            ...
            <div class="pro-price">{{item.skuPrice|priceFormat}}元</div>
            <div class="pro-num">{{item.skuQuantity}}</div>
            <div class="pro-total pro-total-in">{{item.skuPrice * item.skuQuantity|priceFormat}}元</div>
          </li>
        </ul>
        <div class="order-bar">
          ...
          <div class="order-bar-right">
            <!--待收货订单可以 查看物流、确认收货-->
            <el-button v-if="item.itemStatus==2" style="margin-right: 40px;"  @click="dialogTableVisible = true,trackingInformation(item.id)">查看物流</el-button>
            ...
            <el-button v-if="item.itemStatus==2" style="margin-right: 40px;"  @click="dialogVisible = true">确认收货</el-button>
            <el-dialog
                    title="提示"
            ...
            <el-button v-if="item.itemStatus==3" style="margin-right: 40px;"    @click="toReview(item)">去评价</el-button>
            <span>
              <span class="total-price-title">合计：</span>
              <span class="total-price">{{item.skuPrice * item.skuQuantity|priceFormat}}元</span>
            </span>
          </div>
          <!-- 订单列表END -->
        </div>
      ...
      <div class="content" v-for="(item,index) in allUnpaidOrder" :key="index">
        <ul >
          ...
          <li class="product-list"  v-for="(item1,index1) in item.orderItemEntities" :key="index1">
            ...
            <div class="pro-price">{{item1.skuPrice|priceFormat}}元</div>
            <div class="pro-num">{{item1.skuQuantity}}</div>
            <div class="pro-total pro-total-in">{{item1.skuPrice * item1.skuQuantity|priceFormat}}元</div>
          </li>
        </ul>
        <div class="order-bar">
          <div class="order-bar-left">
          ...
          <div class="order-bar-right">
            ...
            <span>
              <span class="total-price-title">合计：</span>
              <span class="total-price">{{item.totalAmount|priceFormat}}元</span>
            </span>
          </div>
          <!-- 订单列表END -->
        </div>
    ...
    <!-- 订单为空的时候显示的内容 -->
    <div v-else-if="(allOrder==null)||(allOrder.length==0)||(allUnpaidOrder==null&&this.activeName=='0')" class="order-empty">
      <div class="empty">
        <h2>您还没有相关的订单</h2>
...
/*      getOrderStatus(){
      ...
      trackingInformation(orderId){
        ...
        let deliveryTimeFormat = this.dateFormat(newOrder.deliveryTime)
